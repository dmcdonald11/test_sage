![Image](tst_output\docling\docling_artifacts\image_000000_4f05ea6de89ce20493a5d9cc2305a4feb948c7bb794d7b81ee29554ec56b8445.png)
...
The image depicts a step-by-step workflow for processing PDF documents. It begins with a PDF file on the far left, followed by an arrow pointing to three black-and-white icons representing parsed PDF pages. The next stage, labeled "Model Pipeline," is enclosed in a dashed blue rectangle and consists of three columns labeled OCR, Layout Analysis, and Table Structure, each containing three document icons with colorful annotations such as highlighted text, red boxes, and table grids. Arrows connect each stage sequentially from OCR to Layout Analysis, then to Table Structure. After this, an arrow leads to a stack of blank document icons labeled "Assemble results, Apply document post-processing," followed by a final arrow pointing to an icon representing a serialized file format, either JSON or Markdown, symbolized by curly braces and a semicolon. This visual summarizes a pipeline for extracting and structuring information from PDF documents into machine-readable formats.

![Image](tst_output\docling\docling_artifacts\image_000001_0b6a3cd25d375e942c65d4ff1ccf1bb92491db4542267e46db0b6130fb555af9.png)
...
The image shows a side-by-side comparison of two versions of the first page of a research paper titled "DocLayNet: A Large Human-Annotated Dataset for Document-Layout Analysis." Both versions include the title, author names and affiliations, and typical academic sections such as Abstract, CCS Concepts, Keywords, and ACM Reference Format. The left version is a slightly zoomed-out view and includes a figure (Figure 1) displaying four examples of complex page layouts across different document categories, showing annotated layout elements with colored bounding boxes and labels. The right version is a zoomed-in, clearer and more readable view of the same text but without the figure or colored annotations visible. Both pages emphasize key terms like "layout segmentation," "object-detection," "PDF document conversion," and "Machine Learning," and outline the dataset's purpose for improving document layout analysis with human annotation, highlighting diverse page layouts and challenging document elements. The left side includes publication info and a DOI reference that is matched on the right but formatted distinctly. The figure on the left helps visualize the diversity and complexity of page layouts tackled by the dataset.

![Image](tst_output\docling\docling_artifacts\image_000002_862eaa7905f8305ca24cb654314f5c73ad3ef7772269b340b0507fa8004ed79b.png)
...
![Image](tst_output\docling\docling_artifacts\image_000003_b22f3a350efb0fcca7f933c13ff2649fb0187e203ab3ce7257a58a63ff08780d.png)
...
The image consists of three distinct panels labeled A, B, and C. Panels A and B present detailed tables listing various document element classes (Caption, Footnote, Formula, List-item, Page-footer, Page-header, Picture, Section-header, Table, Text, Title) along with corresponding data metrics such as count, percentage of total in training, testing, and validation sets, and triple inter-annotator mean average precision (mAP) values at different Intersection over Union (IoU) thresholds (0.5-0.95, 0.5, 0.75) for several categories (All, Fin, Man, Sci, Law, Pat, Ten). The data is numerical and highlights distribution and annotation performance for each class across datasets. Panel C shows a snippet of JSON code detailing a table that specifies its structure with 12 columns and 14 rows. It includes box coordinates and metadata about the position and span of the text "triple inter-annotator mAP @ 0.5-0.95 (%)" located at column 5 spanning columns 5 through 12 in row 0 with a row span of 1. This JSON snippet likely represents the structured annotation or extraction of the tabular data visible in panels A and B. Overall, the image combines tabular experimental or dataset statistics and the technical representation of their layout annotation.

![Image](tst_output\docling\docling_artifacts\image_000004_4a01b28a8a2637376a483b46952be822d854bd0ea98a4edfdcd5e99ffb7860be.png)
